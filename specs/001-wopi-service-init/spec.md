...
file-service. The user edits and saves; Collabora calls PutFile,
and the service writes the updated content via file-service's
...
3. **Given** a valid access token, **When** Collabora sends a GetFile request, **Then** the service returns the file content from file-service with a 200 status.
4. **Given** a valid access token with edit permission, **When** Collabora sends a PutFile request, **Then** the service writes the content via file-service and returns a 200 status.
...
- How does the system behave when file-service is unavailable during GetFile or PutFile? → 502 Bad Gateway.
...
- **FR-004**: System MUST implement CheckFileInfo returning file metadata (filename, size, owner, user permissions, supported WOPI features) looked up from file-service via `GET /internal/file/:id/meta`.
- **FR-005**: System MUST implement GetFile returning binary file content retrieved from file-service via `GET /internal/file/:id/content`.
- **FR-006**: System MUST implement PutFile persisting updated file content via file-service's store-and-link endpoint (`PUT /internal/file/:id/content`).
...
- **FR-011**: System MUST authorize file access by calling the authorization-evaluation-service (via h2c HTTP or NATS, configurable) with actorId, privilege (`read` or `update-content`), and the document's authorizationPolicyId. The auth call MUST be wrapped with a circuit breaker.
- **FR-012**: System MUST look up document metadata (externalID, authorizationPolicyId, mimeType, displayName, size) from file-service via the `/internal/file/:id/meta` endpoint.
...
- **Document** (via file-service): File metadata in Alkemio. Key attributes: id (UUID), externalID (SHA3-256 hash), displayName, mimeType, size, authorizationId (JSON field name, mapped to `AuthorizationPolicyID` in domain model). Looked up via file-service `GET /internal/file/:id/meta`.
...
- **SC-001**: A user can open a document from Alkemio in Collabora Online, edit it, and save changes that are persisted back via file-service — full round-trip works end to end.
...
- Q: How does the WOPI service read/write files? → A: Via file-service private endpoints by document ID. GetFile uses `GET /internal/file/:id/content`. PutFile uses `PUT /internal/file/:id/content`.
- Q: Where does the WOPI service get document metadata? → A: Via file-service `GET /internal/file/:id/meta`. No direct database access.
...
- file-service is deployed and its private endpoints (`/internal/file/...`) are reachable within the K8s cluster. Document metadata and file content are accessed exclusively through file-service — no direct Alkemio database access.
...
- File content is not cached by the WOPI service — every GetFile reads from file-service, and every PutFile writes to it.
- Auth env vars (`AUTH_SERVICE_URL`, `NATS_URL`, `AUTH_BREAKER_*`) are shared with file-service and can be defined by the same K8s configmap.
